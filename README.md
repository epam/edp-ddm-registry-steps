...
```gherkin
      Тоді результат запиту містить наступні значення "2022" у полі "year"
```
...
```gherkin
     Тоді дата модель повертає json з файлу "features/data/search-conditions/status/status_name_read_all.json"
```

* @Тоді("дата модель повертає json, що містить точно наступні дані, ігноруючи невказані:")
    * Нижче цього кроку необхідно у вигляді строки вказати очікувані дані отримані у відповідь на запит  
      Приклад:

```gherkin 
     Тоді дата модель повертає json, який містить точно наступні дані, ігноруючи невказані:
      | [{"code": "TYPE", "name": "Тип"}] |
```  

* @Тоді("дата модель повертає точно заданий json з файлу {string}, ігноруючи невказані")
    * Для змінної `string` необхідно вказати шлях до json файлу із очікуваним результатом Приклад:

```gherkin 
     Тоді дата модель повертає точно заданий json з файлу "features/data/search-conditions/type/type_read_all.json", ігноруючи невказані
``` 
