...
```
* Після встановлення empty-template при створенні репозиторію реєстра registry-regulations створюється така 
  структура для автоматизованих тестів:
  * autotests
    * features
    * files
      * keys
    * properties
      * platform.json (empty template)
      * users.json (empty template)
...
```
     Коли користувач "auto-user" виконує запит пошуку "academic-year-name-contains" з параметрами
       | name | 2022 |
```
...
* @Тоді("дата модель повертає json з файлу {string}")
    * Для змінної `string` необхідно вказати шлях до json файлу із очікуваним результатом Приклад:
...
## В абзаці передумов які виконуються перед запуском сценарію, або в степах блоку Тоді використовуються наступні кроки:
...
* @Коли("виконується фільтрація результатів запиту {string} за параметрами")
    ...
    * Нижче цього кроку необхідно у вигляді таблиці вказати назву параметрів за якими хочемо фільтрувати та Їх
      значення  
      `| параметр | значення |`  
      Приклад:
...
* @Дано("користувач {string} успішно увійшов у кабінет посадової особи")
    ...
    * `user.json` файл необхідно додати до репозиторію `registry-tests` за наступним
      шляхом `\src\test\resources\properties\users.json`
    * дані атрибутів заповнюються згідно даних ключа, який необхідно додати до репозиторію `registry-tests` за наступним
      шляхом `\src\test\resources\files\keys\Key-6.dat`   
...
* @Коли("бачить передзаповнені поля із даними$")
    * Дані для заповнення форми повинні бути додані в тесті у вигляді таблиці з наступною структурою
        ...
        * type - тип елементу форми, можливі значення `radiobutton`, `RADIOBUTTON`, `checkbox`, `CHECKBOX`, `input`
          , `INPUT`, `select`, `SELECT`, `datetime`, `DATETIME`
...
Функціонал: Створити endpoint для заповнення поля 'Тип' з підтримкою EQUALS
 ...
 Структура сценарію: [EQUALS] Перевірка запиту пошуку за полем 'Тип' та порівняння значень в обраних параметрах результату
  ...
  Коли користувач "auto-user" виконує запит пошуку "application-type-equals" з параметрами
   | code | <code> |
